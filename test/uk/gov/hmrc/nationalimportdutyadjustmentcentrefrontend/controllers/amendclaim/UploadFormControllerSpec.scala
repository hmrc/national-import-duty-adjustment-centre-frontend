...
class UploadFormControllerSpec extends ControllerSpec with TestData {
  ...
  override protected def beforeEach(): Unit = {
    ...
    when(progressView.apply(any())(any(), any())).thenReturn(HtmlFormat.empty)
  }
  ...
  def theProgressViewBackLink: NavigatorBack = {
    val captor = ArgumentCaptor.forClass(classOf[NavigatorBack])
    verify(progressView).apply(captor.capture())(any(), any())
    captor.getValue
  }
  ...
  "onProgress" should {
    ...
    "redirect when uploading a duplicate file" in {
      ...
      withCacheAmendAnswers(completeAmendAnswers.copy(uploads = Seq(uploadAnswer)))
