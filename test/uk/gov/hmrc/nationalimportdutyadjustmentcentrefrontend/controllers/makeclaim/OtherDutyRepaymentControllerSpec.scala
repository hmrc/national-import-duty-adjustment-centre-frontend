...
class OtherDutyRepaymentControllerSpec extends ControllerSpec with TestData {
  ...
  "GET" should {
    ...
    "display page when cache has answer" in {
      withCacheUserAnswers(UserAnswers(otherDutyRepayment = Some(otherDutyRepaymentAnswer)))
      val result = controller.onPageLoad()(fakeGetRequest)
      status(result) mustBe Status.OK
  ...
  "POST" should {
    ...
    val validRequest = postRequest(
      "actuallyPaid" -> otherDutyRepaymentAnswer.actuallyPaid,
      "shouldPaid"   -> otherDutyRepaymentAnswer.shouldHavePaid
    )
    ...
    "update cache and redirect when valid answer is submitted" in {
      ...
      theUpdatedUserAnswers.otherDutyRepayment mustBe Some(otherDutyRepaymentAnswer)
