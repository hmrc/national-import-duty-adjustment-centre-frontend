...
class CustomsDutyRepaymentControllerSpec extends ControllerSpec with TestData {

  private val page         = mock[DutyRepaymentPage]
  private val formProvider = new DutyPaidFormProvider

  private def controller =
    new CustomsDutyRepaymentController(
  ...
  "GET" should {
    ...
    "display page when cache has answer" in {
      withCacheUserAnswers(UserAnswers(customsDutyRepayment = Some(customsDutyRepaymentAnswer)))
      val result = controller.onPageLoad()(fakeGetRequest)
      status(result) mustBe Status.OK
  ...
  "POST" should {
    ...
    val validRequest = postRequest(
      "actuallyPaid" -> customsDutyRepaymentAnswer.actuallyPaid,
      "shouldPaid"   -> customsDutyRepaymentAnswer.shouldHavePaid
    )
    ...
    "update cache and redirect when valid answer is submitted" in {
      ...
      theUpdatedUserAnswers.customsDutyRepayment mustBe Some(customsDutyRepaymentAnswer)
