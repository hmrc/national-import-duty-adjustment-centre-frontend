...
class SignedOutPageViewSpec extends UnitViewSpec {

  private val page = instanceOf[SignedOutPage]

  private val view: Html = page()

  "SignedOutPage" should {

    "have correct title" in {
      view.title() must startWith(messages("signed_out.title"))
    }

    "have correct heading" in {
      view.getElementsByTag("h1").text() mustBe messages("signed_out.title")
    }
