...
@Singleton
class ImportVatRepaymentController @Inject() (
  ...
  def onPageLoad(): Action[AnyContent] = identify.async { implicit request =>
    data.getAnswers map { answers =>
      val preparedForm = answers.importVatRepayment.fold(form)(form.fill)
  ...
  def onSubmit(): Action[AnyContent] = identify.async { implicit request =>
    form.bindFromRequest().fold(
      ...
      value =>
        data.updateAnswers(answers => answers.copy(importVatRepayment = Some(value))) map {
          updatedAnswers => Redirect(navigator.nextPage(ImportVatRepaymentPage, updatedAnswers))
        }
    )
  }

  private def page(form: Form[DutyPaid])(implicit request: IdentifierRequest[_]) =
    repaymentPage(form, routes.ImportVatRepaymentController.onSubmit(), "importVatPaid")
