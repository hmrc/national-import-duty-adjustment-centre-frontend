...
class RepaymentTypeController @Inject() (
  ...
  navigator: Navigator,
  repaymentTypePage: RepaymentTypePage
  ...
  def onPageLoad(): Action[AnyContent] = identify.async { implicit request =>
    data.getAnswers map { answers =>
      val preparedForm = answers.repaymentType.fold(form)(form.fill)
      Ok(repaymentTypePage(preparedForm))
    }
  }

  def onSubmit(): Action[AnyContent] = identify.async { implicit request =>
    form.bindFromRequest().fold(
      formWithErrors => Future(BadRequest(repaymentTypePage(formWithErrors))),
      value =>
        data.updateAnswers(answers => answers.copy(repaymentType = Some(value))) map {
          updatedAnswers => Redirect(navigator.nextPage(RepaymentTypePage, updatedAnswers))
        }
    )
  }
