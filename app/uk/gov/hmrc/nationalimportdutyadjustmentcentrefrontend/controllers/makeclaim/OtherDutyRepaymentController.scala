...
@Singleton
class OtherDutyRepaymentController @Inject() (
  ...
  def onPageLoad(): Action[AnyContent] = identify.async { implicit request =>
    data.getAnswers map { answers =>
      val preparedForm = answers.otherDutyRepayment.fold(form)(form.fill)
  ...
  def onSubmit(): Action[AnyContent] = identify.async { implicit request =>
    form.bindFromRequest().fold(
      ...
      value =>
        data.updateAnswers(answers => answers.copy(otherDutyRepayment = Some(value))) map {
          updatedAnswers => Redirect(navigator.nextPage(OtherDutyRepaymentPage, updatedAnswers))
        }
    )
  }

  private def page(form: Form[DutyPaid])(implicit request: IdentifierRequest[_]) =
    repaymentPage(form, routes.OtherDutyRepaymentController.onSubmit(), "otherDutyPaid")
