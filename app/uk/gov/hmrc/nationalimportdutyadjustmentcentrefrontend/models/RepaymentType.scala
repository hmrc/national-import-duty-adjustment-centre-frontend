...
object RepaymentType extends Enumerable.Implicits {
  ...
  case object TaxAccount extends WithValue("TaxAccount") with RepaymentType
  case object Bacs       extends WithValue("BACS") with RepaymentType

  val values: Seq[RepaymentType] = Seq(TaxAccount, Bacs)

  def options(form: Form[_], name: String)(implicit messages: Messages): Seq[RadioItem] = values.map {
    value =>
      RadioItem(
        value = Some(value.toString),
        content = Text(MessageKey.apply("repaymentType", value.toString)),
